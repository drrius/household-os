...
The user authorized dependency integration on September 5. Current main is now integrated here, including reviewed PR56: browser subscriptions are reconciled with active server registration and paused rows expose Reconnect. The two related findings (3938693869 and 3939705306) now have their implementation dependency in this branch. Twenty-three focused sign-out, sign-in and push-enrollment tests pass; hosted combined browser/database verification remains pending. Live device delivery is still unverified.
