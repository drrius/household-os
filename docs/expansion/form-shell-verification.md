...
Document unload uses `beforeunload` where supported. App Router Back/Forward now uses an application history index and a capture listener that runs before the router: cancelled traversals return to the existing entry without unmounting the edited form. The tracker preserves Next history metadata and adds no sentinel entries. Pending saves block traversal; accepted discards continue normally. Full document unload remains subject to browser activation and mobile lifecycle restrictions. Direct programmatic route changes outside history traversal are not intercepted; no persistent/offline draft storage is introduced.

Eight focused Chromium/mobile Safari cases pass for expense/routine Back and Forward cancellation and acceptance, successful saves/redirects, and same-page anchors. Targeted lint and TypeScript checking pass. CI must verify the complete application with the tracker installed in the root layout.
...
## In-flight saves are not discardable

Finding 3939692699 is addressed: while a protected save is pending, Cancel and same-tab internal navigation are blocked before any discard prompt. The form announces that the save cannot be canceled and asks the user to wait. Browser unload also requests confirmation while pending, subject to browser lifecycle restrictions; an already-issued server command cannot be canceled by closing the page. Completion and failure restore normal navigation/dirty-state behavior.

Full verification again passed 273 tests/build. All 60 production browser cases passed, including the held-request test that attempts Cancel and another route, verifies no discard dialog and unchanged location, then releases the response. The first test run matched the fixture's unrelated output status; the final assertion specifically identifies the save announcement. Evidence: `/tmp/discard-pending-verify.log`, `/tmp/discard-pending-fixture-build.log`, `/tmp/discard-pending-e2e-final.log`.
