...
Full 398 tests, lint, type checks and production build pass. All 27 browser cases pass across Chromium, desktop WebKit and mobile Safari, covering pristine fields, dirty drafts, reverting, selection-only edits/labels, new identity, validation retry, same-page reopening, file removal, pending upload and uploaded-path preservation. One upload request remains one request across refresh. Evidence: `/tmp/home-pristine-verify.log`, `/tmp/home-pristine-fixture-build.log`, `/tmp/home-pristine-e2e.log`.
...
Codex 3939931901 identified that the first implementation also froze new-record choices before any edit. The correction preserves only the initial record/ID while accepting current options until a real draft or submission begins. Dirty new forms retain their matching choice labels, and reverting releases the waiting update without rotating the ID. Full 398 tests/build and 33 browser cases pass. Evidence: `/tmp/new-home-choices-verify.log`, `/tmp/new-home-choices-fixture-build.log`, `/tmp/new-home-choices-e2e.log`.
