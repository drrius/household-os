...
Codex3939931901 identified that the first implementation also froze new-record choices before any edit. The correction preserves only the initial record/ID while accepting current options until a real draft or submission begins. Dirty new forms retain their matching choice labels, and reverting releases the waiting update without rotating the ID. Full398 tests/build and33 browser cases pass. Evidence: `/tmp/new-home-choices-verify.log`, `/tmp/new-home-choices-fixture-build.log`, `/tmp/new-home-choices-e2e.log`.
